...
const credentials = {
    key: fs.readFileSync('./config/sslcert/hostkey.pem'),
    cert: fs.readFileSync('./config/sslcert/hostcert.pem'),
}
...
app.use('/:port/:testKey/confluence', function (req, res) {
	...
	if(testKey === 'low'){
		const webDriver = require('./selenium-scripts/A9998.js')
		webDriver.fillA9998(portName, testKey, callbackA9998)
	}else if (testKey === 'high'){
		const webDriver = require('./selenium-scripts/A9999.js')
