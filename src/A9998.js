...
function fillA9998(portName, testKey, callbackA9998) {
    require('chromedriver');
    const { Builder, By, Key, until } = require('selenium-webdriver');    
    const testRunner = require('./TestJigRunner');
    let driver = new Builder().forBrowser('chrome').build();
    driver.manage().window().maximize();

    const rootXpath = '//*[@id=\"main-content\"]/div[5]/div/div[1]/div/div/table/tbody';       
    
    driver.get('https://confluence.syft.com:8443/display/BUILDS/Universal+Test+Jig+A9998+Ion+Guide');
    ...
    driver.wait(until.elementLocated(By.xpath(rootXpath)), 30000).then(function () {
        ...
        function callbackRunTest(result) {                    
            ...
            console.log('Test completed_A9998.js');
            callbackA9998('Test completed');
        }
    }).catch(function(){
